...
> 🔀 **먼저 타겟을 판정하세요 (대원칙 §2)**. ChatGPT 안에서 gpt-image로 바로 생성 = **웹 UI 경로 → 아래 JSON**. 사용자가 **gpt-image-2 API·Codex `$imagegen`·배치(jsonl)** 로 쓸 프롬프트를 요청 = **킷 포맷이 원본** → 포맷 A(라벨 6섹션) 또는 포맷 B(화보 콤마형 단문) **본문 + 끝 `AR x:y` 토큰**, 네거티브는 전면 긍정형. 두 경로를 뭉뚱그리지 마세요.

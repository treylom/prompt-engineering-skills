...
> **OFC 산출 게이트 (재현 검증 시 2항)** — ① 생성된 프롬프트에 **검증 행동형 동사 0**(`verify`·`double-check`·`재검증하라` 류 — 단 *과업 절차*로서의 검증은 대상 아님, Part 2.5.2 경계 2) ② **OFC 필수 칸 전부 충족**(핵심 산출·근거·한계·판정 + 목적별 확장). 두 항 모두 통과해야 산출 GREEN.

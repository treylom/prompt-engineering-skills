...
⚠️ **경계 1 — 계약 문구에 행동 동사 ❌**: 계약은 **답의 목차**를 정할 뿐 **행동을 지시하지 않는다.** `"verify"`·`"double-check"`·`"re-examine"` 류 동사가 **계약 칸 문구**에 들어가면 그 순간 다시 행동 지시가 되어 공식 권고를 위반한다 — **칸 이름과 채울 내용만** 적는다.

🔴 **경계 2 — 「자기검증」과 「과업 절차」를 구분하라** (혼동 시 품질이 깨진다)

| 구분 | 예 | 처리 |
|---|---|---|
| **자기 산출 재검증** (공식이 빼라는 것) | "네 답을 검증하라" · "응답 전 재확인" · "서브에이전트로 검증하라" | **제거** → 출력 형식 계약으로 전환 |
| **과업 절차 그 자체** (대상 ❌) | 팩트체크의 *"출처 3개 이상 확인 · 교차 검증 수행"* · 코드리뷰의 *"전 파일 스캔"* · 리서치의 *"1차 자료 대조"* | **유지** — 이건 *무엇을 하는 일인가*의 정의다 |

**문자열이 같아도 역할이 다르다.** `QuickFactCheck` 템플릿의 `<Instructions>` 안 "교차 검증 수행"은 **팩트체크라는 작업의 내용**이지 모델에게 자기 답을 재검토하라는 지시가 아니다 — 빼면 팩트체크가 팩트체크가 아니게 된다. 공식 원문의 *"verification instructions"* 는 **자기 산출에 대한 재검증**을 가리킨다.

👉 **판별법 3축** (리터럴 검색은 **후보 추출용**이고, 판정은 아래 3축을 확인한 뒤에 한다)

| 축 | 제거 대상 (자기검증) | 대상 아님 |
|---|---|---|
| **ⓐ 대상** | **자기 답**을 검토 | **외부 주장·자료**를 검토 = 과업 절차 |
| **ⓑ 시점** | 답 **완성 뒤** · 응답 직전 | 판정을 **만들기 전** (작업 단계) |
| **ⓒ 극성** | 재검증을 **요구** | 재검증을 **금지** = 오히려 준수 |

**보조 판별**: 그 지시를 빼도 *작업의 정의*가 유지되나? **유지되면 자기검증**(제거), **무너지면 과업 절차**(유지).

⚠️ **ⓒ가 특히 함정이다** — `Do not review your own answer` 는 `review` 리터럴에 **1히트**로 잡히지만 **자기검증을 금지하는 문장**이라 위반이 아니라 **준수**다. 부호가 반대인데 문자열은 같다. **검증 어휘 grep 만으로 「위반 N건」을 세면 준수문까지 위반으로 계상되어, 규칙을 적용할수록 산출이 나빠진다.**

...
프롬프트 코드블록을 출력한 다음, **반드시 아래 5가지 옵션 메뉴를 그대로 제시하고 사용자 선택을 기다린다.** 이미지 선행 게이트를 탄 경우에도 이 단계는 면제되지 않는다(v3.2.1까지 이 문장이 약해 이미지 경로에서 메뉴가 누락되던 실증 — 2026-07-29 재경님 ChatGPT Works 재현). 유일한 면제 = `--batch` 인자 또는 AUTO 모드(봇 자율 흐름).

...
description: Use when generating AI image prompts — 원본 = 공냥 프롬프트 킷 v4(gpt-image-2 텍스트 프롬프트 계약 — 포맷 A/B·철칙·jsonl·검증기, skills/image-prompt-kit/ vendor 원문). 웹 UI(ChatGPT/Gemini) JSON 간이 경로는 legacy 부록. /prompt 커맨드가 이미지 타겟 감지 시 references/full.md 전체 로드.

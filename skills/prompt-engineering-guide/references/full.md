...
> 📐 **역할 구분** — 아래 `<Instructions>` 의 「출처 3개 이상 확인 · 교차 검증 수행」은 **팩트체크라는 과업의 정의**이지 *모델에게 자기 답을 재검증하라는 지시*가 아닙니다. 따라서 Claude Opus 5 의 「검증 지시 제거」 권고 **대상이 아니며 그대로 유지**합니다(빼면 팩트체크가 팩트체크가 아니게 됩니다). 자기 산출 재검증에 해당하는 지시만 제거 대상이고, 그 요구는 `<OutputFormat>` — 즉 **출력 형식 계약**으로 이미 표현돼 있습니다. 판별법·경계 = `skills/claude-fable-5-prompt-strategies.md` Part 2.5.2 경계 2.
